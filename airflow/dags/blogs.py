...
# ==============================================
DEFAULT_CONFIG = {
    "BASE_URL": "https://api.spaceflightnewsapi.net/v4/blogs/",  # Endpoint oficial
    "LIMIT": 500,                   # Límite máximo por request (API)
    "STATE_FILE": "state/state_blogs.json",  # Ruta para guardar estado de paginación
...
def load_state(state_file):
    ...
    Args:
        state_file (str): Ruta del archivo JSON (ej: 'state/state_blogs.json').
...
def fetch_blogs(base_url, limit, offset):
...
def Extract_and_save_raw_csv(**kwargs):
    ...
    Retorna vía XComs:
        - 'raw_file': Ruta del CSV generado (ej: 'raw/2024-01-01_blogs.csv').
    ...
    while len(data) < config["MAX_ARTICLES"]:
        try:
            ...
            print(f"Obtenidos {received} blogs | Offset actual: {current_offset}")
    ...
    output_file = f"{config['RAW_DIR']}/{timestamp}_blogs_raw.csv"
...
def Transform_and_save_staging_csv(**kwargs):
    ...
    Retorna vía XComs:
        - 'staging_file': Ruta del CSV transformado (ej: 'staging/2024-01-01_blogs.csv').
    """
    ti = kwargs['ti']
    config = DEFAULT_CONFIG
    ...
    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    staging_file = f"{config['STAGING_DIR']}/{timestamp}_blogs_staging.csv"
...
# ==============================================
with DAG(
    dag_id='spaceflight_blogs_etl',
    description='ETL para blogs de Spaceflight News API',
